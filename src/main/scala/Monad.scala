...
object Monad {
  ...
  trait ToMonadOps {
    implicit def toMonadOps[F[_]](target: F[_])(implicit tc: Monad[F]): Ops[F] = new Ops[F] {
      val self = target
  ...
  object ops {
    implicit def toAllMonadOps[F[_]](target: F[_])(implicit tc: Monad[F]): AllOps[F] = new AllOps[F] {
      val self = target
      val typeClassInstance = tc
    }
  }
...
trait MonadLaws[F[_]] {
  ...
  def leftIdentity[A, B](a: A, f: A => F[B]): IsEq[F[B]] =
    F.flatMap(pure(a))(f) =?= f(a)
    //pure(a).flatMap(f) =?= f(a)

  def rightIdentity[A](fa: F[A]): IsEq[F[A]] =
    F.flatMap(fa)(a=>pure(a)) =?= fa
