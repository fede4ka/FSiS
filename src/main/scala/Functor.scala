...
// @typeclass from simulacrum gives ops, implicit summoning.
 trait Functor[F[_]] { self =>
  ...
  // F берётся из трейта, А и В необходимо указать в параметрах функции map
...
object Functor {
  ...
  trait ToFunctorOps {
    implicit def toFunctorOps[F[_]](target: F[_])(implicit tc: Functor[F]): Ops[F] = new Ops[F] {
      val self = target
  ...
  object ops {
    implicit def toAllFunctorOps[F[_]](target: F[_])(implicit tc: Functor[F]): AllOps[F] = new AllOps[F] {
      val self = target
      val typeClassInstance = tc
    }
  }
